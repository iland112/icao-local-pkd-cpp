...
if [ "$DB_TYPE" = "oracle" ]; then
    ...
    if [ "$ORACLE_HEALTH" = "healthy" ]; then
        docker exec icao-local-pkd-oracle bash -c "expdp pkd_user/${ORACLE_PWD}@XEPDB1 directory=DATA_PUMP_DIR dumpfile=pkd_backup.dmp logfile=pkd_backup.log schemas=PKD_USER" 2>/dev/null
        docker cp icao-local-pkd-oracle:/opt/oracle/admin/XE/dpdump/pkd_backup.dmp "$BACKUP_DIR/oracle_backup.dmp" 2>/dev/null
        if [ -f "$BACKUP_DIR/oracle_backup.dmp" ]; then
            ...
            docker exec icao-local-pkd-oracle bash -c "echo 'SELECT COUNT(*) FROM pkd_user.certificate;' | sqlplus -s pkd_user/${ORACLE_PWD}@//localhost:1521/XEPDB1" > "$BACKUP_DIR/oracle_verify.txt" 2>/dev/null
