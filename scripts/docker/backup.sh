...
if docker exec icao-local-pkd-postgres pg_isready -U pkd -d localpkd > /dev/null 2>&1; then
    docker exec icao-local-pkd-postgres pg_dump -U pkd localpkd > $BACKUP_DIR/postgres_backup.sql
    echo "  ✅ PostgreSQL 백업 완료"
else
    echo "  ⚠️  PostgreSQL이 실행 중이지 않습니다"
fi

# Oracle 백업 (if DB_TYPE=oracle)
if [ "$DB_TYPE" = "oracle" ]; then
    ...
    if [ "$ORACLE_HEALTH" = "healthy" ]; then
        docker exec icao-local-pkd-oracle bash -c "expdp pkd_user/${ORACLE_PWD}@ORCLPDB1 directory=DATA_PUMP_DIR dumpfile=pkd_backup.dmp logfile=pkd_backup.log schemas=PKD_USER" 2>/dev/null
        docker cp icao-local-pkd-oracle:/opt/oracle/admin/ORCLCDB/dpdump/pkd_backup.dmp "$BACKUP_DIR/oracle_backup.dmp" 2>/dev/null
        if [ -f "$BACKUP_DIR/oracle_backup.dmp" ]; then
            echo "  ✅ Oracle 백업 완료"
        else
            echo "  ⚠️  Oracle Data Pump 백업 실패 - SQL 덤프로 대체합니다"
            # Fallback: simple SQL export of key tables
            docker exec icao-local-pkd-oracle bash -c "echo 'SELECT COUNT(*) FROM pkd_user.certificate;' | sqlplus -s pkd_user/${ORACLE_PWD}@//localhost:1521/ORCLPDB1" > "$BACKUP_DIR/oracle_verify.txt" 2>/dev/null
            echo "  ℹ️  Oracle 데이터 검증 파일 저장됨"
        fi
    else
        echo "  ⚠️  Oracle이 실행 중이지 않습니다 (status: $ORACLE_HEALTH)"
    fi
fi
