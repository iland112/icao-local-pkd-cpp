...
# Service selection
SERVICE=${1:-all}
if [[ "$SERVICE" != "all" && "$SERVICE" != "pkd-management" && "$SERVICE" != "pa-service" && "$SERVICE" != "pkd-relay" && "$SERVICE" != "monitoring-service" && "$SERVICE" != "ai-analysis" && "$SERVICE" != "frontend" ]]; then
    echo -e "${RED}Usage: $0 [all|pkd-management|pa-service|pkd-relay|monitoring-service|ai-analysis|frontend]${NC}"
    exit 1
fi
...
if [ "$SERVICE" == "all" ] || [ "$SERVICE" == "ai-analysis" ]; then
    deploy_service "ai-analysis" "icao-local-ai-analysis:arm64" "ai-analysis-arm64.tar.gz"
fi
