/**
 * @file relay_crl_provider.cpp
 * @brief ICrlProvider adapter for PKD Relay Service
 */

#include "relay_crl_provider.h"
...
RelayCrlProvider::RelayCrlProvider(repositories::CrlRepository* crlRepo)
    : crlRepo_(crlRepo)
{
    if (!crlRepo_) {
        throw std::invalid_argument("RelayCrlProvider: crlRepo cannot be nullptr");
    }
}

X509_CRL* RelayCrlProvider::findCrlByCountry(const std::string& countryCode) {
    Json::Value crlData = crlRepo_->findCrlByCountryCode(countryCode);
    if (crlData.isNull()) {
        return nullptr;
    }
