...
void AutoProcessingStrategy::processLdifEntries(
    ...
    if (hasSkippedDuplicates && g_uploadServices->queryExecutor()) {
        ...
        try {
            ...
            auto certCounts = g_uploadServices->queryExecutor()->executeQuery(
            ...
            auto crlCountResult = g_uploadServices->queryExecutor()->executeScalar(
            ...
            auto valCounts = g_uploadServices->queryExecutor()->executeQuery(
    ...
    if ((counts.mlCount > 0 || counts.mlscCount > 0) && g_uploadServices->uploadRepository()) {
    ...
    if (!counts.newCscaCountries.empty() && g_uploadServices->validationService()) {
        spdlog::info("AUTO mode: {} new CSCA countries detected — triggering PENDING DSC re-validation",
                     counts.newCscaCountries.size());
        try {
            auto revalResult = g_uploadServices->validationService()->revalidatePendingDscForCountries(
...
void AutoProcessingStrategy::processMasterListContent(
    ...
    if (g_uploadServices->uploadRepository()) {
        g_uploadServices->uploadRepository()->updateStatistics(uploadId,
            stats.cscaNewCount, 0, 0, 0,
            stats.mlscCount, stats.mlCount);
        g_uploadServices->uploadRepository()->updateProgress(uploadId,
    ...
    if (stats.cscaNewCount > 0 && g_uploadServices->validationService()) {
        spdlog::info("AUTO mode: {} new CSCAs from Master List — triggering full PENDING DSC re-validation",
                     stats.cscaNewCount);
        try {
            auto revalResult = g_uploadServices->validationService()->revalidateDscCertificates();
            if (revalResult.success && revalResult.totalProcessed > 0) {
                spdlog::info("AUTO mode: PENDING DSC re-validation complete — {} processed, {} valid, {} still pending ({}s)",
                    revalResult.totalProcessed, revalResult.validCount, revalResult.pendingCount,
                    revalResult.durationSeconds);
