...
void updateUploadStatistics(const std::string& uploadId,
    ...
    // Update status
    g_uploadServices->uploadRepository()->updateStatus(uploadId, status, errorMessage);

    // Update certificate statistics
    g_uploadServices->uploadRepository()->updateStatistics(uploadId, cscaCount, dscCount, dscNcCount, crlCount);
