...
std::pair<std::string, bool> saveCertificateWithDuplicateCheck(
    ...
    if (!g_uploadServices || !g_uploadServices->certificateRepository()) {
        spdlog::error("[CertUtils] g_uploadServices or certificateRepository is null");
        return std::make_pair(std::string(""), false);
    }

    return g_uploadServices->certificateRepository()->saveCertificateWithDuplicateCheck(
...
bool trackCertificateDuplicate(
    ...
    return g_uploadServices->certificateRepository()->trackCertificateDuplicate(
...
bool incrementDuplicateCount(
    ...
    return g_uploadServices->certificateRepository()->incrementDuplicateCount(certificateId, uploadId);
}
...
bool updateCscaExtractionStats(
    ...
    if (!g_uploadServices || !g_uploadServices->uploadRepository()) {
        spdlog::error("[CertUtils] g_uploadServices or uploadRepository is null");
        return false;
    }
...
bool updateCertificateLdapStatus(
    ...
    return g_uploadServices->certificateRepository()->updateCertificateLdapStatus(certificateId, ldapDn);
}
