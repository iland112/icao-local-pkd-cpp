...
#include "upload/common/doc9303_checklist.h"

// processLdifFileAsync() moved to UploadHandler (Phase D3)
// Called via g_uploadServices->uploadHandler()->processLdifFileAsync()
    ...
    try {
        ...
        if (duplicateUpload) {
            ...
            if (canReupload && uploadMode == "FORCE") {
                ...
                spdlog::info("Duplicate LDIF (re-uploadable): existing upload {} (status: {})",
                             duplicateUpload->id, duplicateUpload->status);
                result.success = false;
                result.status = "DUPLICATE_REUPLOADABLE";
                result.errorMessage = "This file was previously uploaded. You can re-upload with confirmation.";
                result.message = "File with hash " + fileHash.substr(0, 16) + "... already exists";
                result.uploadId = duplicateUpload->id;
                return result;
            } else {
                ...
                result.success = false;
                result.status = "DUPLICATE";
                result.errorMessage = "Duplicate file detected. This file is currently being processed.";
                result.message = "File with hash " + fileHash.substr(0, 16) + "... already exists";
                result.uploadId = duplicateUpload->id;
                return result;
            }
    ...
    try {
        ...
        if (duplicateUpload) {
            ...
            if (canReupload && uploadMode == "FORCE") {
                ...
                spdlog::info("Duplicate ML (re-uploadable): existing upload {} (status: {})",
                             duplicateUpload->id, duplicateUpload->status);
                result.success = false;
                result.status = "DUPLICATE_REUPLOADABLE";
                result.errorMessage = "This file was previously uploaded. You can re-upload with confirmation.";
                                    ...
                                    const std::vector<uint8_t>& fileContent, LDAP* ld)
{
    ...
    std::string crlId = g_uploadServices->crlRepository()->save(result.uploadId, countryCode, issuerDn,
...
{
    // Delegates to UploadHandler::processLdifFileAsync() via g_uploadServices (Phase D3)
    spdlog::info("[UploadService] Delegating LDIF async processing to UploadHandler");
    if (g_uploadServices && g_uploadServices->uploadHandler()) {
        g_uploadServices->uploadHandler()->processLdifFileAsync(uploadId, content);
    } else {
        spdlog::error("[UploadService] Cannot process LDIF: g_uploadServices or uploadHandler not available");
    }
}
