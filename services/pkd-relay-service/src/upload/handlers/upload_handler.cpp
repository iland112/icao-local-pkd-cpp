...
// Common utilities
#include "upload/common/ldif_types.h"
#include "upload/common/main_utils.h"
#include "upload/common/progress_manager.h"
#include "upload/common/certificate_utils.h"
#include "upload/common/x509_metadata_extractor.h"
#include "upload/processing_strategy.h"
#include "upload/ldif_processor.h"
...
void UploadHandler::processLdifFileAsync(const std::string& uploadId, const std::vector<uint8_t>& content, bool resumeMode) {
    // Atomic check+register: prevent duplicate threads AND enforce concurrent limit under single mutex
    {
        ...
        if (s_activeProcessingCount.load() >= MAX_CONCURRENT_PROCESSING) {
            ...
            g_uploadServices->uploadRepository()->updateStatus(uploadId, "FAILED",
    ...
    std::thread([uploadId, content, resumeMode]() {
        ...
        if (!g_uploadServices->ldapStorageService()) {
        ...
        try {
            ...
            if (g_uploadServices && g_uploadServices->uploadRepository()) {
                g_uploadServices->uploadRepository()->updateStatus(uploadId, "PROCESSING", "");
                if (!resumeMode) {
                    g_uploadServices->uploadRepository()->updateProgress(uploadId, totalEntries, 0);
                }
            ...
            if (g_uploadServices && g_uploadServices->uploadRepository()) {
                g_uploadServices->uploadRepository()->updateStatus(uploadId, "FAILED", e.what());
            }
        }
    ...
    std::thread([uploadId, content, trustAnchorPath, resumeMode]() {
        ...
        try {
            ...
            if (content.empty() || content[0] != 0x30) {
                ...
                g_uploadServices->uploadRepository()->updateStatus(uploadId, "FAILED", "Invalid CMS format");
                g_uploadServices->uploadRepository()->updateStatistics(uploadId, 0, 0, 0, 0, 0, 0);
                // LDAP connection auto-released by LdapConnectionGuard destructor
                // ProcessingSlotGuard destructor handles cleanup automatically
                return;
            }
            ...
            if (!cms) {
                ...
                if (p7) {
                    ...
                    if (certs) {
                        ...
                        for (int i = 0; i < numCerts; i++) {
                            ...
                            if (totalCerts % 10 == 0) {
                                ...
                                if (totalCerts % 50 == 0) {
                                    g_uploadServices->uploadRepository()->updateProgress(uploadId, totalCertsInML, savedCount);
                                }
                            }

                            // Save certificate using Repository Pattern
                            auto [certId, isDuplicate] = g_uploadServices->certificateRepository()->saveCertificateWithDuplicateCheck(
                            ...
                            if (!certId.empty()) {
                                if (isDuplicate) {
                                    ...
                                    if (ld) {
                                        std::string ldapDn = g_uploadServices->ldapStorageService()->saveCertificateToLdap(ld, certType, countryCode,
                                                                                    subjectDn, issuerDn, serialNumber,
                                                                                    fingerprint, derBytes);
                                        if (!ldapDn.empty()) {
                                            g_uploadServices->certificateRepository()->updateCertificateLdapStatus(certId, ldapDn);
                                            ldapStoredCount++;
                                        }
                                    }
                                }
                    ...
                    g_uploadServices->uploadRepository()->updateStatus(uploadId, "FAILED", "CMS/PKCS7 parsing failed");
                    g_uploadServices->uploadRepository()->updateStatistics(uploadId, 0, 0, 0, 0, 0, 0);
                    // LDAP connection auto-released by LdapConnectionGuard destructor
                    return;
                }
                                    ...
                                    if (derLen > 0) {
                                        ...
                                        if (!certId.empty()) {
                                            if (isDuplicate) {
                                                ...
                                                g_uploadServices->certificateRepository()->trackCertificateDuplicate(certId, uploadId, "ML_FILE", countryCode, "", "");
                                            } else {
                                                cscaCount++;
            ...
            // Update statistics
            g_uploadServices->uploadRepository()->updateStatus(uploadId, "COMPLETED", "");
            g_uploadServices->uploadRepository()->updateStatistics(uploadId, cscaCount, dscCount, 0, 0, 1, 1);
            int finalTotal = totalCertsInML > 0 ? totalCertsInML : totalCerts;
            g_uploadServices->uploadRepository()->updateProgress(uploadId, finalTotal, cscaCount + dscCount);

            // Save validation statistics to DB
            if (g_uploadServices->validationRepository()) {
                ...
                g_uploadServices->validationRepository()->updateStatistics(uploadId, valStats);
