...
std::string saveCertificateToLdap(LDAP* ld, const std::string& certType,
    ...
    // v1.4.24: Verify certificate is valid DER format before LDAP save
    const unsigned char* certPtr = certBinary.data();
    X509* testCert = d2i_X509(nullptr, &certPtr, certBinary.size());
    if (!testCert) {
        spdlog::error("Certificate binary is not valid DER format (size: {}): {}",
                     certBinary.size(), ERR_error_string(ERR_get_error(), nullptr));
        return "";
    }
    X509_free(testCert);
    spdlog::debug("Certificate binary validated as DER format (size: {})", certBinary.size());
...
int main(int argc, char* argv[]) {
    ...
    spdlog::info("====== ICAO Local PKD v1.4.24 VERIFY-CERT-DER-FORMAT ======");
