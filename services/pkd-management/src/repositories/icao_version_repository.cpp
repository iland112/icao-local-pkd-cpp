    ...
    try {
        ...
        if (dbType == "oracle") {
            ...
            query =
                ...
                "  CASE "
                "    WHEN u.original_file_name IS NOT NULL AND REGEXP_SUBSTR(u.original_file_name, 'icaopkd-00[123]-[a-z]+-(\\d+)', 1, 1, NULL, 1) IS NOT NULL THEN "
                "      TO_NUMBER(REGEXP_SUBSTR(u.original_file_name, 'icaopkd-00[123]-[a-z]+-(\\d+)', 1, 1, NULL, 1)) "
                "    ELSE 0 "
            ...
            query =
                ...
                "  v.file_version as detected_version, "
                "  CASE "
                "    WHEN u.original_file_name ~ 'icaopkd-00[123]-[a-z]+-(\\d+)' THEN "
                "      substring(u.original_file_name from 'icaopkd-00[123]-[a-z]+-(\\d+)')::int "
                "    ELSE 0 "
                "  END as uploaded_version, "
                "  COALESCE(to_char(u.upload_timestamp, 'YYYY-MM-DD HH24:MI:SS'), 'N/A') as upload_timestamp "
                "FROM ( "
