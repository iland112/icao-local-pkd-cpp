...
/**
 * @brief Normalize country code to ISO 3166-1 alpha-2 (uppercase, 2 letters)
 *
 * External clients (e.g., MRZ-based systems) often send 3-letter ISO 3166-1 alpha-3
 * country codes (KOR, USA, JPN). LDAP's `c=` attribute requires 2-letter alpha-2 codes.
 *
 * @param code Country code (2 or 3 letters)
 * @return Uppercase 2-letter alpha-2 code, or original if already 2 letters or unmapped
 */
inline std::string normalizeCountryCodeToAlpha2(const std::string& code) {
    ...
    // Not alpha-3, return as-is
    if (upper.length() != 3) return upper;

    // ISO 3166-1 alpha-3 to alpha-2 mapping (complete list + ICAO/MRTD special codes)
    static const std::unordered_map<std::string, std::string> alpha3ToAlpha2 = {
        ...
        // ICAO/MRTD special codes (Doc 9303 Part 3)
        {"GBD","GB"},{"GBN","GB"},{"GBO","GB"},{"GBP","GB"},{"GBS","GB"},
        {"UNA","UN"},{"UNK","XK"},{"UNO","UN"},
        {"XXA","XX"},{"XXB","XX"},{"XXC","XX"},{"XXX","XX"},
    };

    auto it = alpha3ToAlpha2.find(upper);
    if (it != alpha3ToAlpha2.end()) {
        return it->second;
    }

    // Unknown 3-letter code, return as-is (will likely fail in LDAP but at least logged)
    return upper;
}
