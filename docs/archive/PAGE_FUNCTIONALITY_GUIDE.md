...
**프로젝트**: FASTpass® SPKD (ICAO Local PKD) v2.37.0
**작성일**: 2026-03-18
**총 페이지 수**: 29개
...
### 추가 페이지
25. [CSR 관리 (CsrManagement)](#25-csr-관리-csrmanagement)
26. [DSC 등록 승인 (PendingDscApproval)](#26-dsc-등록-승인-pendingdscapproval)
27. [인증서 품질 보고서 (CertificateQualityReport)](#27-인증서-품질-보고서-certificatequalityreport)
28. [API 클라이언트 접근 신청 (ApiClientRequest)](#28-api-클라이언트-접근-신청-apiclientrequest)
29. [EAC 대시보드 (EacDashboard)](#29-eac-대시보드-eacdashboard)
...
| 권한 | 설명 | 사이드바 메뉴 |
|------|------|-------------|
| cert:read | 인증서 조회 | 인증서 조회 |
| cert:export | 인증서 내보내기 | - |
| pa:verify | PA 검증 실행 | PA 검증 수행 |
| pa:read | PA 이력 조회 | 검증 이력 |
| pa:stats | PA 통계 조회 | PA 검증 통계 |
| upload:read | 업로드 이력 조회 | 업로드 이력 |
| upload:write | 파일 업로드 | 파일 업로드, 인증서 업로드 |
| report:read | 보고서 조회 | 인증서 통계, DSC Trust Chain, CRL 보고서, 표준 부적합 DSC, 인증서 품질 |
| ai:read | AI 분석 결과 조회 | AI 인증서 분석 |
| sync:read | 동기화 상태 조회 | 동기화 상태 |
| icao:read | ICAO 상태 조회 | ICAO 버전 상태 |
| api-client:manage | API 클라이언트 관리 | - (관리자 전용) |

> **v2.37.0 변경사항**: 기존 `pa:read` 권한을 `pa:read`(이력)와 `pa:stats`(통계)로 분리. `api-client:manage` 권한 추가. 총 10개 → 12개.
...
| JWT 필요 (권한 기반) | FileUpload, CertificateUpload, Profile (모든 페이지는 사이드바 `permission` 필드에 의해 메뉴 노출 제어) |
...
- 사용 페이지: DscNcReport, CrlReport, PADashboard, AiAnalysisDashboard, CertificateSearch, CertificateQualityReport
...
PA 검증에서 자동 추출된 DSC 인증서의 등록 승인/거부 관리 페이지. PA 검증 시 SOD에서 추출된 DSC는 자동 등록 대신 `pending_dsc_registration` 테이블에 임시 저장되며, 관리자가 확인 후 승인/거부한다.

- **통계 카드**: 대기(PENDING), 승인(APPROVED), 거부(REJECTED), 전체 수량
- **필터**: 상태(PENDING/APPROVED/REJECTED), 국가
- **테이블**: 국가(국기), Subject DN, 상태 배지, 등록 시간, 검토자
- **승인 다이얼로그**: 코멘트 입력 후 승인 → `certificate` 테이블 INSERT + LDAP 저장
- **거부 다이얼로그**: 코멘트 입력 후 거부
- **상세 모달**: 인증서 메타데이터 (Subject DN, Issuer DN, 유효기간, 지문, 알고리즘)

### 사용 API

| 메서드 | 엔드포인트 | 용도 |
|--------|-----------|------|
| GET | `/api/certificates/pending-dsc` | 대기 목록 (페이지네이션, 필터) |
| GET | `/api/certificates/pending-dsc/stats` | 통계 (PENDING/APPROVED/REJECTED) |
| POST | `/api/certificates/pending-dsc/{id}/approve` | 승인 (JWT 필수) |
| POST | `/api/certificates/pending-dsc/{id}/reject` | 거부 (JWT 필수) |
...
| **인증** | 불필요 (공개) |
| **위치** | 보고서 & 분석 섹션 > 인증서 보고서 하위 |

### 기능 설명

ICAO Doc 9303 기반 인증서 품질(준수/미준수/경고) 분석 보고서 대시보드.

- **요약 카드**: 전체 인증서, 준수(Compliant), 미준수(Non-Compliant), 경고(Warning) 수량
- **카테고리별 위반 차트**: 알고리즘, 키 크기, KeyUsage, 확장 필드, 유효기간 카테고리별 막대 차트
- **국가별 품질 분포**: 국가별 준수/미준수/경고 수량
- **인증서 유형별 분포**: CSCA/DSC/MLSC 등 유형별 품질 분류
- **위반 상세 테이블**: 필터(국가, 유형, 상태), 페이지네이션
- **CSV 내보내기**: 품질 보고서 CSV 다운로드

### 사용 API

| 메서드 | 엔드포인트 | 용도 |
|--------|-----------|------|
| GET | `/api/certificates/quality/report` | 품질 보고서 데이터 |
...
외부 시스템에서 API 클라이언트 접근을 신청하는 공개 페이지. 로그인 없이 접근 가능하며, Layout(사이드바/헤더) 없이 독립적으로 렌더링된다.

- **신청 폼**: 신청자 정보(이름, 소속, 전화번호, 이메일), 사유, 클라이언트 이름, 설명
- **기기 유형**: SERVER/DESKTOP/MOBILE/OTHER 선택
- **권한 선택**: 10가지 API 권한 체크박스
- **IP 화이트리스트**: SERVER/DESKTOP 유형 시 허용 IP 입력
- **제출 완료**: 신청 ID 표시 + 안내 메시지
- **PII 암호화**: 신청자 개인정보(이름, 소속, 전화번호, 이메일) AES-256-GCM 암호화 저장

### 사용 API

| 메서드 | 엔드포인트 | 용도 |
|--------|-----------|------|
| POST | `/api/auth/api-client-requests` | 접근 신청 제출 |
...
| **인증** | 필요 (관리자 전용, 실험적 기능) |
| **위치** | 보고서 & 분석 섹션 (`appConfig.enableEacMenu` 활성 시) |

### 기능 설명

BSI TR-03110 기반 CVC(Card Verifiable Certificate) 인증서 관리 실험적 대시보드.

- **통계 카드**: 전체 CVC, CVCA, DV(국내/해외), IS(검사 시스템) 수량
- **CVC 업로드**: 바이너리 CVC 파일 업로드 + 미리보기
- **인증서 목록**: 국가 필터, 유형별 색상 배지 (CVCA/DV/IS)
- **인증서 상세**: ASN.1 구조 TreeViewer, CHAT 권한 디코딩
- **체인 검증**: IS → DV → CVCA 3-depth 체인 검증 (chainValid 표시)
- **인증서 삭제**: 2단계 확인 후 삭제
...
| 메서드 | 엔드포인트 | 용도 |
|--------|-----------|------|
| GET | `/api/eac/statistics` | EAC 통계 |
| GET | `/api/eac/countries` | 국가 목록 |
| GET | `/api/eac/certificates` | CVC 인증서 검색 |
| GET | `/api/eac/certificates/{id}/chain` | 체인 검증 |
| POST | `/api/eac/certificates/upload` | CVC 업로드 |
| POST | `/api/eac/certificates/preview` | CVC 미리보기 |
| DELETE | `/api/eac/certificates/{id}` | CVC 삭제 |
