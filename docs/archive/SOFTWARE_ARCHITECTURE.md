...
- **CSR Management**: ICAO PKD CSR 생성 (RSA-2048 + SHA256withRSA), 외부 CSR Import, ICAO 발급 인증서 등록
- **DSC Pending Approval**: PA 검증에서 추출된 DSC 자동등록 → 관리자 승인 워크플로우
- **PII Encryption**: AES-256-GCM 인증 암호화 (개인정보보호법 제29조 — CSR 개인키, PII 필드)
...
graph TD
    subgraph "React Application Structure"
        ...
        subgraph "Page Components (29 Pages)"
            ...
            CsrMgmt[CsrManagement.tsx<br/>CSR 관리]
            PendingDsc[PendingDscApproval.tsx<br/>DSC 등록 승인]
            ApiClientMgmt[ApiClientManagement.tsx<br/>API 클라이언트]
            ApiClientReq[ApiClientRequest.tsx<br/>API 접근 요청]
            AiAnalysis[AiAnalysisDashboard.tsx<br/>AI 분석]
            CrlReport2[CrlReport.tsx<br/>CRL 보고서]
            TrustChain[TrustChainValidationReport.tsx<br/>Trust Chain 보고서]
            CertQuality[CertificateQualityReport.tsx<br/>인증서 품질]
            EacDash[EacDashboard.tsx<br/>EAC 인증서]
            ProfilePage[Profile.tsx<br/>사용자 프로필]
        end
...
graph TB
    subgraph "보안 레이어 (v2.37.0)"
        ...
        subgraph "데이터 레이어"
            ...
            PII["PII 암호화<br/>AES-256-GCM<br/>CSR 개인키/개인정보"]
...
- ✅ **PII Encryption (개인정보보호법 제29조)**: AES-256-GCM 인증 암호화 — CSR 개인키, CSR PEM, 여권번호, 이름, 연락처 등
- ✅ **CSR Private Key Security**: AES-256-GCM 암호화 저장, API 응답에 개인키 미포함, 삭제 시 영구 파기
...
- 🔍 ICAO PKD LDAP V3 직접 연결 — 인증서 자동 동기화 (2026년 3월~ ICAO 전환)
- 🔍 ICAO PKD REST API 연동 — HTTP 기반 인증서 다운로드
...
FASTpass® SPKD v2.37.0은 **마이크로서비스 아키텍처**, **Multi-DBMS**, **ICAO 9303 완전 준수**, **CSR 기반 PKD 연계**를 통해 높은 성능, 확장성, 보안성을 제공합니다.

**핵심 강점**:
- ✅ 5개 독립 마이크로서비스 (PKD Management, PA, Relay, Monitoring, AI Analysis)
- ✅ Multi-DBMS 지원 — PostgreSQL 15 / Oracle XE 21c 런타임 전환
- ✅ **ICAO PKD CSR 관리** — RSA-2048 CSR 생성/Import, 발급 인증서 등록, 공개키 매칭 검증
- ✅ **DSC 등록 승인 워크플로우** — PA 추출 DSC 자동등록 → 관리자 승인/거부
- ✅ JWT 인증 + RBAC (12 permissions, pa:stats 포함) + API Key 인증 (X-API-Key) + Dual Audit Logging
- ✅ **PII 암호화** (AES-256-GCM) — CSR 개인키, 여권번호, 개인정보 필드
- ✅ OWASP 보안 강화 완료 (Command Injection 제거, SQL Injection 방지)
- ✅ AI 인증서 포렌식 분석 (Isolation Forest + LOF, 10-category forensic risk scoring)
- ✅ icao::validation 공유 라이브러리 (ICAO 9303 Part 12)
- ✅ DB-LDAP 데이터 일관성 보장 (31,212 인증서, 100% 동기화)
- ✅ C++20 고성능 백엔드 + React 19 모던 프론트엔드 (29 페이지)
- ✅ ARM64 CI/CD 파이프라인 (GitHub Actions → Luckfox 배포)
- ✅ Docker / Podman 기반 유연한 배포 (로컬, Production RHEL 9, ARM64)

---

**Document Created**: 2026-01-20
**Last Rewrite**: 2026-03-18
