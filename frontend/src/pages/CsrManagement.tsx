...
export default function CsrManagement() {
  ...
  const handleImport = async () => {
    ...
    setImportError('');
    try {
      const resp = await csrApiService.import({
        csrPem: importCsrPem.trim(),
        privateKeyPem: importKeyPem.trim(),
        memo: importMemo,
      });
      if (resp.data.success) {
        setImportOpen(false);
        setImportCsrPem(''); setImportKeyPem(''); setImportMemo('');
        fetchList();
      } else {
        setImportError(resp.data.error || 'Import 실패');
      }
    } catch (err: unknown) {
      const axiosErr = err as { response?: { data?: { error?: string } } };
      setImportError(axiosErr.response?.data?.error || 'Import 중 오류 발생');
  ...
  const handleViewDetail = async (id: string) => {
    ...
    try {
      ...
      if (resp.data.success) setSelectedCsr(resp.data.data);
  ...
  const handleExportPem = async (id: string) => {
    try {
      const resp = await csrApiService.exportPem(id);
    ...
    <div className="px-4 lg:px-6 py-4 space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">CSR 관리</h1>
            <p className="text-xs text-gray-500 dark:text-gray-400">ICAO PKD 인증서 서명 요청 (Certificate Signing Request)</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => { setImportOpen(true); setImportError(''); setImportCsrPem(''); setImportKeyPem(''); setImportMemo(''); }}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm rounded-lg transition-colors"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                <tr><td colSpan={6} className="py-8 text-center text-gray-400"><Loader2 className="w-5 h-5 animate-spin mx-auto" /></td></tr>
              ) : csrList.length === 0 ? (
                <tr><td colSpan={6} className="py-8 text-center text-gray-400 text-sm">생성된 CSR이 없습니다</td></tr>
              ) : csrList.map((csr) => (
                <tr key={csr.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  ...
                  <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                    {csr.created_at?.substring(0, 19).replace('T', ' ')}
                  </td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <button onClick={() => handleExportPem(csr.id)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded" title="PEM 다운로드">
            ...
            <div className="flex gap-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button key={p} onClick={() => setPage(p)}
                  className={`px-2 py-0.5 text-xs rounded ${p === page ? 'bg-indigo-600 text-white' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{p}</button>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50" onClick={() => setGenerateOpen(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700">
              ...
              <button onClick={() => setGenerateOpen(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"><X className="w-4 h-4 text-gray-500" /></button>
            </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <input id="csr-country" type="text" maxLength={2} value={genForm.countryCode}
                      ...
                      className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500" placeholder="KR" />
                  </div>
                  <div>
                    ...
                    <input id="csr-org" type="text" value={genForm.organization}
                      ...
                      className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500" placeholder="Government of Korea" />
                  </div>
                  <div>
                    ...
                    <input id="csr-cn" type="text" value={genForm.commonName}
                      ...
                      className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500" placeholder="Korea PKD" />
                  </div>
                  <div>
                    ...
                    <input id="csr-memo" type="text" value={genForm.memo}
                      ...
                      className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500" placeholder="ICAO PKD 등록용" />
                  </div>
                </>
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-green-600"><ShieldCheck className="w-5 h-5" /><span className="text-sm font-medium">CSR 생성 완료</span></div>
                  <div className="relative">
                    <pre className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-[10px] font-mono overflow-x-auto max-h-48 overflow-y-auto whitespace-pre-wrap break-all">{genResult.csrPem}</pre>
                    <button onClick={() => handleCopy(genResult.csrPem)} className="absolute top-2 right-2 p-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded hover:bg-gray-100 dark:hover:bg-gray-700" title="복사">
                      {copied ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-gray-500" />}
                    </button>
                  </div>
                  <button onClick={() => handleExportPem(genResult.id)} className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-xs rounded-lg">
            ...
            <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
              <button onClick={() => setGenerateOpen(false)} className="px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 rounded-lg text-gray-700 dark:text-gray-300">닫기</button>
              {!genResult && (
      ...
      {/* Import Dialog */}
      {importOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50" onClick={() => setImportOpen(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">CSR 가져오기</h2>
              </div>
              <button onClick={() => setImportOpen(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"><X className="w-4 h-4 text-gray-500" /></button>
            </div>
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-3">
              ...
              <div>
                <label htmlFor="import-csr" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">CSR (PEM)</label>
                <textarea id="import-csr" rows={6} value={importCsrPem} onChange={(e) => setImportCsrPem(e.target.value)}
                  className="w-full px-3 py-2 text-[11px] font-mono border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500 resize-none"
                  placeholder="-----BEGIN CERTIFICATE REQUEST-----&#10;...&#10;-----END CERTIFICATE REQUEST-----" />
              </div>
              <div>
                <label htmlFor="import-key" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">개인키 (PEM)</label>
                <textarea id="import-key" rows={6} value={importKeyPem} onChange={(e) => setImportKeyPem(e.target.value)}
                  className="w-full px-3 py-2 text-[11px] font-mono border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500 resize-none"
                  placeholder="-----BEGIN PRIVATE KEY-----&#10;...&#10;-----END PRIVATE KEY-----" />
              </div>
              <div>
                <label htmlFor="import-memo" className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">메모 (선택)</label>
                <input id="import-memo" type="text" value={importMemo} onChange={(e) => setImportMemo(e.target.value)}
                  className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500" placeholder="외부 생성 CSR" />
              </div>
              {importError && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-2.5 text-xs text-red-700 dark:text-red-300">{importError}</div>
              )}
            </div>
            <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
              <button onClick={() => setImportOpen(false)} className="px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 rounded-lg text-gray-700 dark:text-gray-300">취소</button>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50" onClick={() => setDetailOpen(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700">
              ...
              <button onClick={() => setDetailOpen(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"><X className="w-4 h-4 text-gray-500" /></button>
            </div>
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-3">
              {detailLoading ? (
                ...
                <>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div><span className="text-gray-500">Subject DN</span><p className="font-medium mt-0.5">{selectedCsr.subject_dn}</p></div>
                    ...
                    <div><span className="text-gray-500">키 알고리즘</span><p className="font-mono mt-0.5">{selectedCsr.key_algorithm}</p></div>
                    <div><span className="text-gray-500">서명 알고리즘</span><p className="font-mono mt-0.5">{selectedCsr.signature_algorithm}</p></div>
                    <div className="col-span-2"><span className="text-gray-500">공개키 핑거프린트</span><p className="font-mono mt-0.5 text-[10px] break-all">{selectedCsr.public_key_fingerprint}</p></div>
                    <div><span className="text-gray-500">생성자</span><p className="mt-0.5">{selectedCsr.created_by}</p></div>
                    <div><span className="text-gray-500">생성일</span><p className="mt-0.5">{selectedCsr.created_at?.substring(0, 19).replace('T', ' ')}</p></div>
                  ...
                  {selectedCsr.csr_pem && (
                    <div className="relative">
                      ...
                      <pre className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-[10px] font-mono overflow-x-auto max-h-40 overflow-y-auto whitespace-pre-wrap break-all">{selectedCsr.csr_pem}</pre>
                      <button onClick={() => handleCopy(selectedCsr.csr_pem!)} className="absolute top-6 right-2 p-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
                        {copied ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-gray-500" />}
                      </button>
                    </div>
                  )}
                  <button onClick={() => handleExportPem(selectedCsr.id)} className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-xs rounded-lg">
                    <Download className="w-3.5 h-3.5" /> PEM 다운로드
                  </button>
                  ...
                  {selectedCsr.status !== 'ISSUED' && selectedCsr.status !== 'REVOKED' && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-amber-50 hover:bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 text-xs rounded-lg border border-amber-200 dark:border-amber-800 transition-colors"
                    >
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50" onClick={() => setCertRegOpen(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700 bg-amber-50 dark:bg-amber-900/20 rounded-t-lg">
              <div className="flex items-center gap-2"><Award className="w-4 h-4 text-amber-600" /><h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">ICAO 발급 인증서 등록</h2></div>
              <button onClick={() => setCertRegOpen(false)} className="p-1 hover:bg-amber-100 rounded"><X className="w-4 h-4 text-gray-500" /></button>
            </div>
            <div className="px-5 py-4 space-y-3">
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-2.5 text-xs text-blue-700 dark:text-blue-300">
              ...
              <div>
                ...
                <textarea id="cert-pem" rows={10} value={certPemInput} onChange={(e) => setCertPemInput(e.target.value)}
              ...
              {certRegError && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-2.5 text-xs text-red-700 dark:text-red-300">{certRegError}</div>
              )}
            </div>
            <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
              <button onClick={() => setCertRegOpen(false)} className="px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 rounded-lg text-gray-700 dark:text-gray-300">취소</button>
